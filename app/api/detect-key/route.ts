...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${AUDIO_SERVER}/key`, { method: "POST", body: fwd, signal: AbortSignal.timeout(60000) });
    const data = await res.json();
    if (!res.ok) return Response.json({ error: data.error || "Key detection failed" }, { status: 500 });
